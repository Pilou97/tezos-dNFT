...
let transfer (from: address) (to: address) (token_id: token_id) (storage: t) =
...
let get_balance (holder: address) (token_id: token_id) (storage: t) =
...
let add_operator (operator: address) (owner: address) (token_id: token_id) (storage:t) =
    let {metadata; ledger; operators; token_metadata; counter} = storage in
...
let remove_operator (operator: address) (owner: address) (token_id: token_id) (storage:t) = 
    let {metadata; ledger; operators; token_metadata; counter} = storage in
    let operators_set = Big_map.find_opt (owner, token_id) operators in
    let operators_set = match operators_set with
        | None -> Set.empty 
...
let get_operators (owner: address) (token_id: token_id) (storage: t) = 
...
let is_owner (address: address) (token_id: token_id) (storage: t) =
    let {metadata=_; ledger; operators=_; token_metadata=_; counter=_} = storage in
    match Big_map.find_opt token_id ledger with
        | None -> failwith Error.fa2_token_undefined
        | Some owner -> address = owner 

let get_token (token_id: token_id) (storage: t) = Big_map.find_opt token_id storage.token_metadata

let update_token (token_id: token_id) (metadata: metadata) (storage: t) = 
...
let assert_can_transfer_token (address: address) (owner: address) (token_id: token_id) (storage: t) = 
...
let assert_token_defined (token_id: token_id) (storage: t) = 
...
let assert_sufficient_balance (amount: nat) (owner: address) (token_id: token_id) (storage: t) =
...
let assert_can_update_metadata (address: address) (token_id: token_id) (storage: t) =
    // TODO: duplicated code
    let is_owner = is_owner address token_id storage in
    if is_owner then () else failwith Error.fa2_not_owner

let assert_exists (token_id: token_id) (storage: t) =
